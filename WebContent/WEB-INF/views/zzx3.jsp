...
<!-- 		<rect x="2.58" y="14.607" fill="none" class="stroke_color" stroke-miterlimit="10" width="1020" height="80"/> -->
		...
		<rect id="Z01" data-modal="modal-1" x="654.108" y="556.877" fill="#7CFC00"
			class="stroke_color md-trigger toPointer " stroke-miterlimit="10" width="24.357" height="15.238" style="fill:rgb(255, 9, 0);" />
...
<script type="text/javascript">
	$(".transformer1").click(function() {
		window.location.href = 'fzx.do';
	})
	$(".transformer2").click(function() {
		window.location.href = 'yd.do';
	})
</script>
...
<!--获取红色机柜，并且更新状态 -->
	...
	var getRedJiGui=function(){
		var current_red_jigui = [];
		$("rect").each(function(){
		/*	if((typeof($(this).attr('id')) != 'undefined') && ($(this).attr('fill') =='#FF0900')){  //去除undefined
				current_red_jigui.push($(this).attr('id'));
			}
		*/
			console.log($(this).attr('style'));
			if((typeof($(this).attr('id')) != 'undefined') && ($(this).attr('style') == "fill: rgb(255, 9, 0);")){  //去除undefined
				current_red_jigui.push($(this).attr('id'));
			}
			
		})
		return current_red_jigui;
	}
	
	
	var updateRedJiGui=function(){
		var redjigui=getRedJiGui();
		console.log(redjigui);
		$.ajax({
			url : '<%=path%>/updateJiGuiInfo.do',
			type : 'post',
			traditional: true,//属性在这里设置
			data:{"selectedList":redjigui,"location":"zzx"},
			dataType : 'json',
			success:function(result){
				for(var i = 0 ; i < result.length;i++)
				{
	...
	setInterval('updateRedJiGui()',5000);
</script>
